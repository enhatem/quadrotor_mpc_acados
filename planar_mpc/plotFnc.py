...
def plotPos_ref_point_kalman(t,simX, states, save=True):
    plt.style.use('seaborn')

    fig, (ax1, ax2, ax3) = plt.subplots(nrows= 3, ncols = 1, sharex=True)

    ax1.plot(t, simX[1:,0], label='y')
    ax1.plot(t, states[:,0,0], label='y_kalman')

    ax2.plot(t, simX[1:,1], label='z')
    ax2.plot(t, states[:,1,0], label='z_kalman')
    
    ax3.plot(t, R2D(simX[1:,2]), label='phi')
    ax3.plot(t, R2D(states[:,2,0]), label='phi_kalman')
...
def plotSim_ref_point_kalman(simX, states, save=False):
    ...
    ax.plot(states[:,0,0], states[:,1,0], label='traj_kalman')
...
def plotPos_with_imported_traj_kalman(t,simX, states, ref_traj, Nsim, save=False):
    plt.style.use('seaborn')
...
def plotVel_with_imported_traj_kalman(t,simX, states, ref_traj, Nsim, save=False):
    plt.style.use('seaborn')
...
def plotVel_with_imported_traj(t,simX, ref_traj, Nsim, save=False):
    plt.style.use('seaborn')
    ...
    ax1.legend()
    ax1.set_title('States: Velocities')
    ax1.set_ylabel('vy[m]')
...
def plotPos_with_imported_traj(t,simX, ref_traj, Nsim, save=False):
    plt.style.use('seaborn')
...
def plotVel_with_ref_pose(t,simX, Nsim, save=True):
...
def plotVel_with_ref_pose_kalman(t,simX, states, covs, Nsim, save=True):
...
def plotVel_without_vy_vz_references_kalman(t,simX, states, pred, covs, ref_traj, Nsim, save=True):
